...
| **Статус** | В работе. На 2026-04-11 закрыты runtime-level scope isolation, observability, scope-aware read-path для `event_journal`, session-level active scope persistence, DB-enforced RLS slices для `event_journal` + `stats`, а также storage authorization для `sessions` и DB-backed `auth_sessions`; следующим шагом остаётся rollout non-superuser app-role по окружениям и подключение нового auth store в реальные entrypoints. |
