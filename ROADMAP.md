...
| **Статус** | В работе. На 2026-04-13 foundation slice `v2.0` расширен до operator-ready surface: broker-контракт с consumer groups, versioned plugin registry с security-guardrails, воспроизводимый load scenario для broker foundation, web control plane с минимальным UX contract и отдельный Go-backed control-plane projection с endpoint-ами `/api/health`, `/api/scopes`, `/api/control-plane` и мутациями broker/module/plugin при сохранении frontend local fallback. Поверх этого закреплены общий JSON seed, file-backed snapshot, startup-гидратация plugin manifests, канонические demo manifests, runtime reload manifests без рестарта backend, диагностический `/api/health`, вывод operator diagnostics на первый экран web UI и inline-редактирование module/plugin параметров, чтобы backend/frontend не расходились по дефолтам, операторские изменения переживали рестарт, plugin slice опирался на реальные артефакты, а control-plane snapshot можно было не только наблюдать, но и править из dashboard. |
...
- `controlplane/api.go`, `controlplane/http.go`, `controlplane/service.go`, `controlplane/service_test.go`: `/api/health` переведён с простого ping на структурированный operator status с persistence/plugin-hydration полями и snapshot freshness, не ломая frontend-заказчика по полю `ok`.
- `frontend/src/types/control-plane.ts`, `frontend/src/lib/api.ts`, `frontend/src/modules/control-plane/ControlPlaneDashboard.tsx`, `frontend/src/test/control-plane.spec.tsx`, `frontend/tests/e2e/smoke.spec.ts`, `frontend/README.md`: web control plane начал показывать операторские diagnostics из `/api/health` прямо на первом экране; UI различает `backend online` и `local fallback`, а также выводит mode, snapshot freshness, persist path и manifest source/count без ухода в логи.
- `frontend/src/modules/control-plane/ControlPlaneDashboard.tsx`, `frontend/src/test/control-plane.spec.tsx`, `frontend/README.md`, `README.md`: web control plane получил inline-редакторы для module/plugin operator-override полей; теперь dashboard сохраняет `dispatchMode`, `consumerGroup`, `allowedScopes`, `tags`, `latencyBudgetMs`, а также plugin `description/capabilities` через существующий backend patch-contract без JSON-ручной правки.
- `controlplane/api.go`, `controlplane/http.go`, `controlplane/service.go`, `controlplane/*_test.go`, `frontend/src/lib/api.ts`, `frontend/src/modules/control-plane/ControlPlaneDashboard.tsx`, `frontend/src/test/control-plane.spec.tsx`, `README.md`, `frontend/README.md`: control plane получил runtime reload plugin manifests без рестарта backend: backend переиспользует уже настроенный `CONTROL_PLANE_PLUGIN_DIR`, а web dashboard триггерит refresh явной operator-кнопкой и подтягивает свежий snapshot/diagnostics.
...
- [x] `/api/health` отражает операторское состояние backend, а не только базовый ping
- [x] Первый экран web control plane показывает diagnostics `/api/health` и явно различает backend vs local fallback
- [x] Module/plugin operator overrides можно редактировать из web dashboard и это покрыто frontend-тестами
- [x] Plugin manifests можно перечитать в runtime без рестарта `cmd/controlplane`, и это покрыто backend/frontend тестами
...
| 2026-04-13 | `v2.0` дополнён диагностическим `/api/health` для control plane | Зафиксировать operator-facing readiness signal по persistence и plugin hydration без изменения frontend-контракта | Lead Product Architect |
| 2026-04-13 | `v2.0` дополнён выводом operator diagnostics на первый экран web control plane | Зафиксировать, что `/api/health` используется не только как backend JSON endpoint, но и как визуальный readiness signal в UI с fallback-режимом | Lead Product Architect |
| 2026-04-13 | `v2.0` дополнён inline-редактированием module/plugin operator overrides в web control plane | Зафиксировать переход от dashboard-наблюдения к реальным UI-мутациям dispatch/plugin параметров без ручного редактирования snapshot JSON | Lead Product Architect |
| 2026-04-13 | `v2.0` дополнён runtime reload plugin manifests в control plane | Зафиксировать операторский refresh manifest source без рестарта backend и без потери frontend local fallback | Lead Product Architect |
