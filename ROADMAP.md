...
| **Статус** | В работе. На 2026-04-11 закрыты runtime-level scope isolation, observability, scope-aware read-path для `event_journal` и session-level active scope persistence; полноценный DB-enforced RLS остаётся следующим шагом. |
