...
| **Статус** | В работе. На 2026-04-11 закрыты runtime-level scope isolation, observability, scope-aware read-path для `event_journal`, session-level active scope persistence и DB-enforced RLS slices для `event_journal` + `stats`; следующим шагом остаётся расширение storage authorization на остальные scope-bound таблицы и rollout non-superuser app-role по окружениям. |
...
- `databases/rls_status.go`, `databases/db.go`, `databases/cmd/databases/main.go`, `databases/rls_status_test.go`: для RLS добавлена readiness-диагностика; startup-лог и CLI `rls-status` теперь показывают, effective ли policy для текущего DB role или соединение идёт под superuser/BYPASSRLS.
