...
| **Статус** | В работе. На 2026-04-11 закрыты runtime-level scope isolation, observability, scope-aware read-path для `event_journal`, session-level active scope persistence и первый DB-enforced RLS slice для `event_journal`; следующим шагом остаётся расширение storage authorization на app-role / broader scope-bound tables. |
