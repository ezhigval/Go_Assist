...
| **Статус** | В работе. На 2026-04-12 закрыты runtime-level scope isolation, observability, scope-aware read-path для `event_journal`, session-level active scope persistence, DB-enforced RLS slices для `event_journal` + `stats`, storage authorization для `sessions` и DB-backed `auth_sessions`, а также реальное подключение auth store в `telegram` transport; следующим шагом остаётся rollout non-superuser app-role по окружениям и проверка staging/production wiring под non-superuser DB role. |
