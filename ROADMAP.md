...
| **Статус** | В работе. На 2026-04-12 зафиксирован foundation slice: broker-контракт с consumer groups, versioned plugin registry и web control plane для конфигурации broker/module/plugin/scope без backend-зависимости в dev-режиме. |
...
- `plugins/registry.go`, `plugins/registry_test.go`: реализован versioned plugin registry с JSON manifest contract, runtime `process|wasm`, protocol `grpc|stdio`, capability-based resolve, защитой от path escape и `LoadDir("*.plugin.json")`.
