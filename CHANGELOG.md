...
## [2026-04-13] v2.0: persistent control plane state

- [2026-04-13] | `controlplane/service.go`, `controlplane/service_test.go`, `cmd/controlplane/main.go`, `README.md`, `ARCHITECTURE.md`, `ROADMAP.md`, `frontend/README.md` | `cmd/controlplane` переведён на file-backed snapshot: operator mutations теперь атомарно пишутся в `CONTROL_PLANE_STATE_PATH` и переживают рестарт процесса; сервис валидирует snapshot при загрузке и не применяет изменения в память, если запись на диск не удалась | §3 E1, §4 C1, §5 V1, §7
...
## [2026-04-13] v2.0: canonical demo plugin manifests

- [2026-04-13] | `plugins/manifests/*.plugin.json`, `cmd/controlplane/main.go`, `README.md`, `ROADMAP.md`, `frontend/README.md` | В репозиторий добавлены канонические demo manifests для `finance-sync`, `tracker-plan`, `audit-mirror`; `cmd/controlplane` по умолчанию гидратирует plugin slice из `plugins/manifests`, так что operator API опирается на реальные plugin artifacts даже без дополнительного env-конфига | §2 A2, §3 E1, §5 V1, §7

---

## [2026-04-13] v2.0: health diagnostics for control plane
...
## [2026-04-13] v2.0: editable module and plugin overrides in web control plane
...
- [2026-04-12] | `databases/config.go`, `databases/rls_enforcement.go`, `databases/db.go`, `databases/cmd/databases/main.go`, `databases/README.md`, `README.md`, `.env.example`, `config/config.example.yaml`, `ROADMAP.md`, `ARCHITECTURE.md` | Для rollout non-superuser app-role storage readiness переведён в fail-fast режим: `DB_REQUIRE_RLS_EFFECTIVE=true` заставляет startup падать при ineffective RLS, а CLI `rls-status -require-effective` даёт deploy/CI preflight с non-zero exit code | §3 E3, §3 E5, §7
