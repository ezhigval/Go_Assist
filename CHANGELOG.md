...
- [2026-04-11] | `databases/rls_status.go`, `databases/rls_status_test.go`, `databases/db.go`, `databases/cmd/databases/main.go`, `databases/README.md`, `ROADMAP.md` | Для operational readiness v1.5 добавлена диагностика journal RLS: `databases.Start()` и CLI `rls-status` теперь показывают effective ли policy для текущей DB role, и явно предупреждают о superuser/BYPASSRLS bypass | §3 E3, §7
