...
- Operator API: `controlplane/` проецирует broker/module/plugin/scope snapshot в отдельный HTTP surface (`cmd/controlplane`) с маршрутами `/api/health`, `/api/scopes`, `/api/control-plane`, не меняя frontend-контракт и не ломая local fallback path. Дефолтный snapshot вынесен в общий `controlplane/default_snapshot.json`, операторские мутации пишутся в file-backed snapshot (`CONTROL_PLANE_STATE_PATH`), а plugin slice при наличии manifests гидратируется через `CONTROL_PLANE_PLUGIN_DIR`.
